...
function nameWithS() {
  ...
  return got.houses.reduce((acc, house) => {
    acc = acc.concat(
      house.people.map((person) => person.name).filter((name) => name.toLowerCase().includes("s"))
    );
    return acc;
  }, []);
}
...
function nameWithA() {
  ...
  return got.houses.reduce((acc, house) => {
    acc = acc.concat(
      house.people.map((person) => person.name).filter((name) => name.toLowerCase().includes("a"))
    );
    return acc;
  }, []);
}
